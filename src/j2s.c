...
static
int _j2s_json_to_obj(j2s_ctx *ctx, cJSON *json, cJSON *parent,
...
int j2s_json_from_struct(j2s_ctx *ctx, cJSON *json,
...
/* Enum name to value */
static inline
int j2s_enum_get_value(j2s_ctx *ctx, int enum_index, const char *name) {
	j2s_enum *enum_obj;
...
/* Enum value to name */
static inline
const char *j2s_enum_get_name(j2s_ctx *ctx, int enum_index, int value) {
	j2s_enum *enum_obj;
...
static
cJSON *_j2s_enum_to_json(j2s_ctx *ctx, int enum_index) {
	j2s_enum *enum_obj;
	cJSON *root, *item;
...
static
cJSON *_j2s_struct_to_template_json(j2s_ctx *ctx, int struct_index) {
...
static inline
double j2s_obj_get_value(j2s_ctx *ctx, int obj_index, void *ptr) {
	j2s_obj *obj;
	double value;
...
static inline
int j2s_obj_set_value(j2s_ctx *ctx, int obj_index, double value, void *ptr) {
	j2s_obj *obj;
...
static inline
void j2s_extract_array(j2s_obj *obj) {
...
static inline
void *j2s_extract_dynamic_array(j2s_obj *obj, int len, void *ptr) {
...
cJSON *_j2s_obj_to_json(j2s_ctx *ctx, int obj_index, void *ptr) {
	...
	if (obj->type == J2S_TYPE_STRUCT)
		return _j2s_struct_to_json(ctx, obj->struct_index,
					   ptr + obj->offset);
	...
	if (j2s_template_dumping) {
		...
		if (obj->enum_index >= 0) {
			...
			j2s_enum_value *enum_value =
				&ctx->enum_values[enum_obj->value_index];
	...
	if (obj->enum_index >= 0) {
		...
		const char *name = j2s_enum_get_name(ctx, obj->enum_index,
						     (int)value);
...
static
int j2s_json_to_array_with_index(j2s_ctx *ctx, cJSON *json,
	...
	if (query) {
		...
		for (int i = 0; i < size; i++) {
			...
			obj->offset =
				tmp_obj.offset + index * tmp_obj.elem_size;
			item = NULL;

			DBG("handling index array: %s %d/%d\n",
			    obj->name, index, tmp_obj.num_elem);

			/* Query item */
			if (index < tmp_obj.num_elem)
				item = _j2s_obj_to_json(ctx, obj - ctx->objs,
							ptr);
		...
		for (int i = 0; i < size; i++) {
			...
			ret = _j2s_json_to_obj(ctx, item, parent,
					       obj - ctx->objs, ptr, false);
			if (ret < 0)
				break;
		}
	}
...
int _j2s_json_to_obj(j2s_ctx *ctx, cJSON *json, cJSON *parent, int obj_index,
	...
	if (J2S_IS_ARRAY(obj)) {
		...
		if (index_json)
			return j2s_json_to_array_with_index(ctx, json,
							    index_json, parent,
							    obj, ptr, query);
	...
	if (J2S_IS_POINTER(obj)) {
		...
		if (query && !index_json) {
			...
			len_json = _j2s_obj_to_json(ctx, obj->len_index,
						    ptr);
			DASSERT_MSG(len_json, return -1,
				    "failed to query %s\n", len_name);
		...
		if (len_json) {
			len = (int)cJSON_GetNumberValue(len_json);
		} else if (index_json) {
			len = old_len;
		} else {
		...
		if (len != old_len) {
			...
			*buf = j2s_alloc_data(ctx, len * obj->elem_size);
			...
			DBG("re-alloc %s from %d*%d to %d*%d = %p\n",
			    obj->name, old_len, obj->elem_size, len,
			    obj->elem_size, *buf);
		}
	...
	/* Handle struct member */
	if (obj->type == J2S_TYPE_STRUCT)
		return _j2s_json_to_struct(ctx, root, obj->struct_index,
					   ptr + obj->offset, query);
	...
	if (query) {
		...
		if (obj->enum_index >= 0) {
			/* Convert enum value to name */
			const char *name = j2s_enum_get_name(ctx,
							     obj->enum_index,
							     (int)value);
		...
		if (obj->enum_index >= 0) {
			...
			value = (double)j2s_enum_get_value(ctx,
							   obj->enum_index,
							   name);
		} else {
...
int _j2s_json_to_struct(j2s_ctx *ctx, cJSON *json, int struct_index,
	...
	for (child_index = struct_obj->child_index; child_index >= 0;
		...
		DBG("start child: %s (%s) from %p\n",
		    child->name, struct_obj->name, ptr);
		ret = _j2s_json_to_obj(ctx, item, root,
				       child_index, ptr, query);
		DBG("finish child: %s (%s)\n", child->name, struct_obj->name);
		if (ret < 0)
			break;
	}
